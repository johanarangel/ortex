...
def DateDatetime(date):
    return datetime.strptime(date, "%a %dth %B, %Y")

start["date"]= DateDatetime(start["date"])
end["date"]= DateDatetime(end["date"])
...
for k, v in test_data.items():
    ...
    total = v['sales'] + total
    print(f"Total sales for the month {month} es: {total}")